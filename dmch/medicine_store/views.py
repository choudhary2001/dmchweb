    ...
    if request.user.is_superuser or d is not None:
        ...
        if request.method == 'POST':
            ...
            for i in range(1, int(request.POST.get('product_count')) + 1):
                ...
                p = ProductSupply.objects.filter(productdetails_id = product_name).first()
                print(p)
                if p:
...
def get_consumption_and_remaining_quantity(request):
    ...
    if request.user.is_superuser or d is not None:
        ...
        if end_date_str:
            ...
            end_date = None
        a = Supply.objects.filter(departpment=d).order_by('-order_date')
        p = []
        for ap in a:
            ps = ap.products.all()
            department = None
            ...
            for a in ps:
                print(a)
                # if start_date and end_date:
                #     total_quantity = Medicine.objects.filter(product=a,created_at__range=(start_date, end_date)).aggregate(total_quantity=Sum('quantity'))['total_quantity']
                #     p.append({
                ...
                if start_date:
                    ...
                    start_datetime = timezone.make_aware(start_date, timezone.get_current_timezone()).replace(hour=0, minute=0, second=0)
                    end_datetime = start_datetime + timedelta(days=1)
                    # one_end_datetime = start_datetime - timedelta(days=1)
                    total_quantity = Medicine.objects.filter(product=a,created_at__gte=start_datetime, created_at__lt=end_datetime).aggregate(total_quantity=Sum('quantity'))['total_quantity']
                    ...
                    till_remaining = 0
                    till_end = 0
                    if end_total_quantity is not None:
                        if total_quantity is not None:
                            till_today = end_total_quantity - total_quantity
                        till_remaining = a.quantity - till_today
                        till_end = a.quantity - end_total_quantity
